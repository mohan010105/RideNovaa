...
serve(async (req) => {
  ...
  try {
    ...
    const { bookingId, amount, pickupLocation, dropLocation, cabType } = await req.json();
      ...
      line_items: [
        {
          price_data: {
            currency: "inr",
            product_data: {
              name: `Cab Ride - ${cabType || "Sedan"}`,
              description: `${pickupLocation || "Pickup"} → ${dropLocation || "Drop"}`,
            },
